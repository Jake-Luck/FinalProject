...
class Routing(Algorithm):
    ...
    def brute_force(self,
        ...
        best_route = self.generate_route(0, n_routes, num_locations, num_days,
                                    route_length)
        best_evaluation = self.evaluate_route(best_route, num_days, graph)

        for i in range(1, n_routes):  # yikes
            route = self.generate_route(i, n_routes, num_locations, num_days,
                                   route_length)
            evaluation = self.evaluate_route(route, num_days, graph)
