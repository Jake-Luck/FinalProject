...
def generate_route(base_set: list[int], route_number: int, n_routes: int,
                   route_length: int) -> np.ndarray:
    n_factorial = n_routes / (route_length - 1)
    route = np.empty(route_length)
...
def brute_force(num_locations: int, num_days: int,
                graph: np.ndarray) -> np.ndarray:
    route_length = num_locations + num_days - 1

    best_evaluation = float('inf')
    best_route = np.empty(route_length)

    # get (route_length - 1)! Is -1 because all routes end at '0'
    n_routes = 1
    for i in range (2, route_length):
        n_routes *= i

    base_set = list()
    for _ in range(num_days): base_set.append(0)
    for i in range(1, num_locations): base_set.append(i)

    for i in range(n_routes):  # yikes
        temp_set = base_set[:]
        route = generate_route(temp_set, i, n_routes, route_length)
        evaluation = utilities.evaluate_route(route, num_days, graph)
